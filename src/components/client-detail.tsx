...
export function ClientDetailView({ id }: { id: string }) {
  ...
  return <>
    ...
    <div className="detail-columns">
      ...
      <section className="panel detail-section"><h2>Senior reviews</h2>{data.seniorReviews.length ? data.seniorReviews.map(review => <article className="timeline-entry" key={review.id}><div className="section-heading-inline"><span className="status-pill">{label(review.state)}</span><span className="muted">{displayDate(review.due_at)}</span></div><strong>{review.professional_name}</strong><p>{label(review.reason)}</p><p className="muted">{review.requested_decision}</p><Link className="text-link" href={`/issues?issue=${encodeURIComponent(review.issue_id)}`}>Open issue workflow →</Link></article>) : <p className="muted">No senior reviews for this client.</p>}</section>
    </div>
    ...
    <div className="detail-columns">
      ...
      <section className="panel detail-section"><h2>Issues</h2>{data.issues.length ? data.issues.map(issue => <article className="timeline-entry" key={issue.id}><div className="section-heading-inline"><span className={`status-pill ${issue.severity === 'critical' ? 'critical' : ''}`}>{label(issue.state)}</span><time className="muted">{displayDate(issue.created_at)}</time></div><strong>{issue.professional_name}</strong><p>{issue.description}</p><Link className="text-link" href={`/issues?issue=${encodeURIComponent(issue.id)}`}>Manage issue →</Link></article>) : <p className="muted">No issues recorded.</p>}</section>
    </div>
  </>;
}
