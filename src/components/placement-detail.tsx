...
export function PlacementDetail({id}:{id:string}){
 ...
 <div className="page-heading"><div><h1>{data.placement.professional_name}</h1><p className="page-description">{data.placement.role} · {data.placement.client_name}</p></div><button className="button button-primary" onClick={()=>setEditing(!editing)}>{editing?'Back to details':'Edit placement'}</button></div>
