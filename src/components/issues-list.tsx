...
function IssueCard({issue,asOf,onSaved}:{issue:Issue;asOf:string;onSaved:()=>void}){
 ...
 return <article className={`panel issue-card ${issue.escalation&&issue.escalation.state!=='decision_recorded'?'has-escalation':''}`} id={`issue-${issue.id}`}>
  ...
  <h2>{issue.description}</h2><p className="issue-context">{issue.reporter_name} ({issue.reporter_side}) · {stateLabel(issue.category)}</p><Link className="text-link" href={`/placements/${issue.placement_id}`}>{issue.professional_name} · {issue.client_name} →</Link>
...
export function IssuesList(){
 ...
 const items=data?.items??[];const filtered=items.filter(issue=>filter==='monitoring'?issue.state==='monitoring':filter==='escalated'?Boolean(issue.escalation&&issue.escalation.state!=='decision_recorded'):filter==='closed'?issue.state==='verified_closed':issue.state!=='verified_closed');
 const filters:[string,string][]=[['active','Active'],['monitoring','Monitoring'],['escalated','Senior review'],['closed','Verified closed']];
 return <><div className="issue-toolbar"><div className="queue-tabs" aria-label="Issue filter">{filters.map(([value,label])=><button key={value} className={filter===value?'active':''} onClick={()=>setFilter(value)}>{label}</button>)}</div><button className="button button-primary" onClick={()=>setCreating(value=>!value)}>{creating?'Close form':'Report issue'}</button></div>
  ...
  {creating&&placements.data&&<CreateIssueForm placements={placements.data.items} onCancel={()=>setCreating(false)} onSaved={()=>{setCreating(false);void reload();}}/>}
  {data&&<div className="issue-list">{filtered.map(issue=><IssueCard issue={issue} asOf={data.asOf} onSaved={()=>void reload()} key={issue.id}/>)}{!filtered.length&&<div className="panel data-message"><h2>No issues in this view</h2><p>Choose another status or report a new concern.</p></div>}</div>}</>;
}
