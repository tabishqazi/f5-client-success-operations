...
export function ContactOutcomeForm({card,asOf,onCancel,onSaved}:{card:QueueCard;asOf:string;onCancel:()=>void;onSaved:()=>void}){
 ...
 const suggestion=useMemo(()=>eligible.length?suggestFollowUpDate(notes,asOf):null,[notes,asOf,eligible.length]);
 ...
 return <form className="outcome-form" onSubmit={submit}>
  ...
  {suggestion&&<div className={`date-suggestion ${suggestion.withinAllowedWindow?'':'date-suggestion-warning'}`} aria-live="polite"><div><strong>{suggestion.withinAllowedWindow?'Possible follow-up date':'Follow-up date needs review'}</strong><p>“{suggestion.phrase}” means {displayDate(suggestion.date)} from the operating date.</p>{!suggestion.withinAllowedWindow&&<p>The date must be within the next {POLICY.rescheduling.maximumBusinessDays} business days.</p>}</div>{suggestion.withinAllowedWindow&&(confirmedSuggestion!==suggestion.date||outcome!=='rescheduled'||rescheduleDate!==suggestion.date)?<button type="button" className="button button-secondary" onClick={()=>{setOutcome('rescheduled');setRescheduleDate(suggestion.date);setConfirmedSuggestion(suggestion.date);}}>Use {displayDate(suggestion.date)}</button>:suggestion.withinAllowedWindow&&<span className="suggestion-confirmed">Confirmed</span>}</div>}
  </fieldset>
