...
import { suggestFollowUpDate } from '@/domain/follow-up-date';
...
export function ContactOutcomeForm({card,asOf,onCancel,onSaved}:{card:QueueCard;asOf:string;onCancel:()=>void;onSaved:()=>void}){
 ...
 const suggestion=useMemo(()=>suggestFollowUpDate(notes,asOf),[notes,asOf]);
 const canApplySuggestion=eligible.length>0;
 ...
 return <form className="outcome-form" onSubmit={submit}>
  ...
  <fieldset disabled={saving}><legend>Conversation</legend><div className="form-grid compact-grid">
   ...
   {outcome==='rescheduled'&&<label className="form-field">New contact date<input required type="date" min={asOf} max={addBusinessDays(asOf,POLICY.rescheduling.maximumBusinessDays)} value={rescheduleDate} onChange={event=>{setRescheduleDate(event.target.value);setConfirmedSuggestion('');}}/></label>}
  </div><label className="form-field">Conversation notes<textarea required maxLength={2000} rows={3} value={notes} onChange={event=>setNotes(event.target.value)} placeholder={outcome==='no_answer'?'Record the attempt and any useful context.':outcome==='rescheduled'?'Record why the contact moved.':'Record what was discussed and agreed.'}/></label>
  {suggestion&&<div className={`date-suggestion ${suggestion.withinAllowedWindow&&canApplySuggestion?'':'date-suggestion-warning'}`} aria-live="polite"><div><strong>{!canApplySuggestion?'Follow-up date recognized':suggestion.withinAllowedWindow?'Possible follow-up date':'Follow-up date needs review'}</strong><p>“{suggestion.phrase}” means {displayDate(suggestion.date)} from the operating date.</p>{!canApplySuggestion?<p>This card’s next action stays in the issue workflow. Use Manage issue to schedule it.</p>:!suggestion.withinAllowedWindow&&<p>The date must be within the next {POLICY.rescheduling.maximumBusinessDays} business days.</p>}</div>{canApplySuggestion&&suggestion.withinAllowedWindow&&(confirmedSuggestion!==suggestion.date||outcome!=='rescheduled'||rescheduleDate!==suggestion.date)?<button type="button" className="button button-secondary" onClick={()=>{setOutcome('rescheduled');setRescheduleDate(suggestion.date);setConfirmedSuggestion(suggestion.date);}}>Use {displayDate(suggestion.date)}</button>:canApplySuggestion&&suggestion.withinAllowedWindow&&<span className="suggestion-confirmed">Confirmed</span>}</div>}
  </fieldset>
